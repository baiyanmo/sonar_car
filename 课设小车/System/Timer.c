...
void Timer_Init(void)
{
	...
	TIM_TimeBaseInitStructure.TIM_Period = 7199;					//计数周期，即ARR的值，更新间隔100us
...
// 外部声明软件PWM更新函数
extern void PWM_SoftwareUpdate(void);

/* 定时器中断函数 - 用于超声波测距和软件PWM */
void TIM1_UP_IRQHandler(void)
{
	if (TIM_GetITStatus(TIM1, TIM_IT_Update) == SET)
	{
		// 超声波测距
		extern uint16_t sonar_time_counter;
		if (GPIO_ReadInputDataBit(GPIOA, GPIO_Pin_11) == 1)
		{
			sonar_time_counter++;
		}
		
		// 软件PWM更新（舵机控制）- 每100us调用一次
		PWM_SoftwareUpdate();
